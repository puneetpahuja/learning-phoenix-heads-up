defmodule HeadsUpWeb.EffortLive do
  ...
  def handle_event("add", %{"quantity" => quantity_str}, socket) do
    socket = update(socket, :responders, &(&1 + String.to_integer(quantity_str)))

    {:noreply, socket}
  end

  def handle_event("set-minutes", %{"minutes" => minutes_str}, socket) do
    socket = assign(socket, :minutes_per_responder, String.to_integer(minutes_str))
